...
###############################################################################
# Tokenizes strings using the 'twokenize' module, but also splits up
...
def split_contractions(tokens):
    ...
    for token in tokens:
        ...
        elif len(token)>1 and token[-1] in '\'!"$%^&*(,./<>?;#:@~[]{}+_' and re.match('[A-Za-z0-9]', token[-2]):
            new_tok = token[:-1]
            new_token_list.append(token[-1])
        else:
