...
%\author{Matthew Rowe and Leon Derczynski}% <-this % stops a space

\author{\IEEEauthorblockN{Matthew Rowe\IEEEauthorrefmark{1} and
Leon Derczynski\IEEEauthorrefmark{2}\\}
\IEEEauthorblockA{\IEEEauthorrefmark{1}School of Computing and Communications, Lancaster University, Lancaster UK\\}
\IEEEauthorblockA{\IEEEauthorrefmark{2}}Department of Computer Science, University of Sheffield, Sheffield UK}
...
However investigating how named entities (i.e. proper nouns) appear and spread has yet to be explored, largely due to the computationally intractable nature of performing large-scale entity extraction.
In this paper we present, to the best of our knowledge, one of the first pieces of work to examine the diffusion of named entities on social media (using Reddit as our case study platform).
We first investigate how named entities can be accurately recognised and extracted from discussion posts, using an automated approach based upon conditional random fields and brown clustering.
...
Our findings have important implications for researchers studying language and influence, and for digital marketers who wish to understand entity-level influence dynamics.
\end{abstract}
...
\begin{enumerate}
	\item A method to recognise and extract named entities found with discussion posts based upon conditional random fields and brown clustering, together with an evaluation of this method.
...
In section \ref{sec:datasets_ner} we explain the preparation of the Reddit dataset for our experiments - including down-sampling of 100 subreddits - and the named entity recognition (NER) methodology that we employed.
...
While initially conducted over newswire~\cite{tjong2003introduction}, the older tools tend not to perform so well on modern text types, such as tweets and other short social media text~\cite{derczynski2013microblog}.
...
As we will describe below, we also required corpora from which we could \emph{train} our named entity recogniser - and also assess its performance - therefore we also used the following three datasets:
...
The result that we come to at the end of this stage is a rich dataset for tracking entity diffusion within and across communities.
...
Conversely, recall reveals how well the extraction is performing, and is important to tracking a range of entities.\footnote{N.b. It has often been more challenging to achieve high recall in social media texts than high precision~\cite{ritter2011named,derczynski2015analysis}.}
...
Hence, we used training data composed from newswire and twitter equal parts, with the newswire and twitter brown clusters for annotating the named entities across the 100 sampled subreddits.\footnote{N.b. When predicting entity diffusion we used the top-500 entities which were manually verified for accuracy, and were found to be valid entities.}
...
We have presented a method to detect named entities within Reddit posts that uses Conditional Random Fields and brown clustering.
