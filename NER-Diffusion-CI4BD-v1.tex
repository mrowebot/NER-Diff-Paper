...
The Reddit dataset is very large, and to apply our diffusion models for the first time could prove computationally intense to the point of becoming infeasible.
To this end, we reduced the volume of content analysed.
As the goal is to analyse and model behaviour within communities, we can pare down the whole dataset without damaging the objects being studied by select a subset of subreddits, rather than comments.
Each subreddit is a complete self-enclosed community for our purposes.

We selected the subreddits to examine by drawing from those that were most popular.
Selections were made based on {\tt http://redditlist.com/}, a website that ranks communities by recent activity, subscriber count and also current growth rate.
In total, we selected 100 subreddits.

After this, we unpacked the Reddit dataset and filtered out posts not belonging to our queried subset of subreddits.
This led to {\bf how many docs?}, compared to {\bf and how many here?} in the entire dataset.

{\bf figures for raw dataset (\# per year; total volume over time; volume-rank graph of subreddits; possible ref to appx) }

\subsection{Named Entity Recognition for Reddit}
...
Entity mentions are extracted through named entity recognition (NER).
...
In addition, it has almost always been more challenging to achieve high recall in social media texts than high precision~\cite{ritter2011named,derczynski2015analysis}.
...
To give a great penalty to false negatives, and favour recall, we set $\beta=2$.
...
\hline
Entity-propagations ($p_{u}^{E}$)  & $0.730 / 0.713(\pm0.095)$ & \\
Interactions-based ($p_{u}^{I}$) & $0.501 / 0.560(\pm0.102)$ & \\
Community-homophily ($p_{u}^{C}$) & & \\
