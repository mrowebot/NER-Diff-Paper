...
An entity propagation occurs between $v$ to $u$ when when the latter cites $e$ after being exposed to it by the former (as per Definition \ref{def:exposure}), hence a count of how many actions propagate between $v$ and $u$ can be recorded in $A_{v2u}$.
From this, the influence probability between $v$ and $u$ based on such \emph{propagation} can be calculated as follows, where $A_v$ is how many times $v$ has cited an entity:

\begin{equation}
p^{A}_{v,u} = \frac{A_{v2u}}{A_v}
\end{equation}




\subsubsection{Influence from Interactions}
In this paper we extend this formulation

Influence probability from interactions:
\begin{equation}
p^{I}_{v,u} = \frac{\vert \{ p_u : p_v \in P_v, p_u \in P_u, p_u \rightarrow p_v \} \vert}{\vert \{ p_u : p_u \in P_u, p_u \rightarrow . \} \vert}
\end{equation}

\subsubsection{Influence from Community-Homophily}
Influence probability from community homphily
\begin{equation}
p^C_{v,u} = \frac{\vert C_u \cap C_v \vert}{\vert C_u \cup C_v \vert}
\end{equation}

For learning the pairwise (i.e. $u,v$) action propagation counts ($A_{v2u}$) we use the same algorithm as from \cite{goyal2010learning} by partitioning the top-500 entities into 80\%:20\% training/testing segments.
